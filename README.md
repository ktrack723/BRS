...
### ③ 판 도중에 손댈 수 있는 건 무전 하나다

대화가 굴러가는 동안 화면 오른쪽 위에 **📻 무전 개입** 버튼이 있다. 배급은
**텍스팅 1회 · 토킹 1회**, 그게 전부다(`js/points.js`의 `RADIO`).
...
무전은 대화 기록(transcript)에 아예 들어가지 않는다 — 요원 몫의 **무전 원장**에만 남아
...
```bash
npm test        # 규칙 전량 (143항목) — 수치 · 데이터 · 스키마 · 하네스 · 무전 · 하이어아키 감사
