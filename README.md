...
| 🟦 **user** | 유저 인풋 (모두 캐시됨) | 스타일링 · 동기부여 · 코칭 · 무전 |
...
구조도에 없던 칸이 하나 늘었다 — 🟦 **무전**이다. 코칭과 같은 색(유저 인풋)이고 같은 자리
(고객의 귀)로 들어가지만, 들어가는 **때**가 다르다. 코칭은 자리에 앉기 전에 한 덩이로 가고,
무전은 자리가 굴러가는 도중에 꽂힌다. 그것 말고는 구조도 그대로다.
...
7. **B · 텍스팅 4구간 → 토킹 5구간** — 대화가 굴러가고, 매 구간 무드·러브가 오르내린다.
   굴러가는 도중에 **📻 무전**을 딱 한 번씩 때릴 수 있다 — 텍스팅 1회 · 토킹 1회.
8. **C · 후일담** — 성사 여부와 후일담.
...
손댈 수 있는 건 넷뿐이다 — **스타일링 · 동기부여 · 코칭 · 무전.** 넷 다 점수가 없다.
...
| 💗 **러브 포인트** | 타겟이 고객을 원하게 됐는가. **후일담이 읽는 유일한 숫자다** | 0–20 | 2 | ▼ −1 / **▲ +1~+4** |
...
심판이 코칭 판에 ▲를 23% 주던 시절에는 12가 맞았고, 43%를 주는 지금은 20이다
(`tests/calibrate.mjs`의 실측 기준선 참고). 심판이 다시 얌전해지면 이 숫자도 같이 내린다.
