...
4. **A-1 · 미용실** — 한 문장으로 고객의 **외모**를 통째로 덮어쓴다. 거울 앞에서 고객이 대꾸한다.
5. **A-2 · 취조실** — 형광등 하나, 의자 하나. 고객의 **성격**을 통째로 덮어쓰고, 여기서 **시공이 확정된다**
   (두 주문이 한 호출로 함께 나간다).
6. **B · 코칭 하달** — 고객에게만 들어가는 명령 한 덩이.
...
| 파일 | 하는 일 |
|---|---|
| `js/prompts.js` | **카트리지.** 네 블록 + 반응 하나의 프롬프트와 스키마 전부. 여기 없는 프롬프트는 없다 |
...
판 하나에 **구간당 2콜 + A 1콜 + C 1콜 + 반응 3콜 = 23콜 내외**다.
반응 3콜은 준비 단계에서만 나가고 캐시도 붙지 않는다 — 짧고, 서로 독립이고, 아무 데도 안 흘러간다.
...
```bash
npm test        # 규칙 전량 (109항목) — 수치 · 데이터 · 스키마 · 하네스 · 하이어아키 감사
