...
4. **A · 스타일링 / 동기부여** — 두 문장으로 고객의 **외모**와 **성격**을 통째로 덮어쓴다.
5. **B · 코칭 하달** — 고객에게만 들어가는 명령 한 덩이.
6. **B · 텍스팅 4구간 → 토킹 5구간** — 대화가 굴러가고, 매 구간 무드·러브가 오르내린다.
...
| 레버 | 언제 | 채점? | 실제로 하는 일 |
|---|---|---|---|
| ✂️ **스타일링** | 출격 전 | ❌ | 고객 **외모** 시트를 통째로 다시 쓴다. 3D 아바타도 같이 바뀐다 |
| 🔥 **동기부여** | 출격 전 | ❌ | 고객 **성격** 시트를 통째로 다시 쓴다. 어떤 인간으로 그 자리에 앉을지가 여기서 정해진다 |
| 🎧 **코칭** | 출격 전 | ❌ | 대화 프롬프트에 **명령**으로 박힌다. 비우면 고객은 준비 없이 제 시트대로만 움직인다 |
| 📻 **무전** | 판 도중 | ❌ | 대화를 세우고 고객 이어폰에 명령을 꽂는다. **페이즈마다 한 번** |
...
| 게이지 | 하는 일 | 시작 | 한 걸음 |
|---|---|---|---|
| 🌡 **무드 포인트** | 이 자리가 굴러가는가. **0이 되면 그 자리는 거기서 끝난다** (남은 구간을 안 돈다) | 50 | ±12 |
| 💗 **러브 포인트** | 타겟이 고객을 원하게 됐는가. **후일담이 읽는 유일한 숫자다** | 12 | ±8 |
...
| 파일 | 하는 일 |
|---|---|
| `js/prompts.js` | **카트리지.** 네 블록의 프롬프트와 스키마 전부. 여기 없는 프롬프트는 없다 |
| `js/points.js` | 코드가 들고 있는 수치 전부. 무드·러브, 증감 폭, 페이즈. LLM 없이 단독 테스트된다 |
| `js/engine.js` | 하네스. B를 구간마다 돌리고 C를 부른다. 무전 게이트도 여기 있다. **DOM을 모른다** |
| `js/couples.js` | 상설 의뢰 47건 + 스키마 검증 |
| `js/game.js` | 화면·입력. 구조도의 다섯 칸을 그대로 화면으로 옮긴 것 |
...
판 하나에 **구간당 2콜 + A 1콜 + C 1콜 = 20콜 내외**다.
...
```bash
npm test        # 규칙 전량 (117항목) — 수치 · 데이터 · 스키마 · 하네스 · 무전 · 하이어아키 감사
npm run audit   # 구조도가 코드에 아직 살아 있는가 (「폐지된 것」 절이 핵심이다)
npm run browser # 실제 브라우저 E2E — three.js·썸네일 94개·다섯 화면·두 게이지·무전 개입·후일담
npm run responsive  # 9개 뷰포트 × 8개 화면 — 레이아웃 이탈 · 터치 타깃 · 명암비 AA
...
| 미용실 · 취조실 · 정문 3화면 · 대면 상황 생성 | 준비 단계의 반응 확인 호출들 |
