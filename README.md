...
B-1. 텍스팅 & 토킹 — 대화 생성
     [static] 타겟 외모 · 성격 · 성장환경 · 취향
   + [user]   코칭 내용
   + [user]   무전 내용        ← 페이즈마다 한 번. 대화 도중에 꽂힌다
   + [user]   현장 무전 내용    ← 판 전체 한 번. 물리 지원이 그대로 실행된다
...
| 누르면 | 벌어지는 일 |
|---|---|
| ① **대화가 선다** | 다음 줄 경계에서 말풍선이 멈춘다. 구간 중간이면 **뒤에 있던 대사는 없던 것이 된다** |
| ② **회선이 열린다** | 명령을 적는 칸과, 그게 고객 프롬프트에 어떻게 박히는지가 같이 뜬다 |
| ③ **송출하면 박힌다** | 다음 생성 호출의 `messages`에 「반드시 이행」으로 실린다 |
...
(현장 무전도 같다 — 채점되는 것은 두 사람이 그 사건에 대고 입 밖에 낸 반응뿐이다).
무전은 대화 기록(transcript)에 아예 들어가지 않는다 — 요원 몫의 **개입 원장**에만 남아
사후 검토 화면에 뜬다. 구간이 잘렸으면 **실제로 오간 데까지만 다시 판정한다.**
하지 않은 말은 채점되지 않는다.
...
| 파일 | 하는 일 |
|---|---|
| `js/prompts.js` | **카트리지.** 다섯 호출(A-1·A-2·B-1·B-2·C) + 반응·무전의 프롬프트와 스키마 전부. 여기 없는 프롬프트는 없다 |
| `js/points.js` | 코드가 들고 있는 수치 전부. 무드·러브 눈금, 러브 ▲의 폭을 정하는 규칙 둘, 페이즈, 무전 배급. LLM 없이 단독 테스트된다 |
| `js/engine.js` | 하네스. B를 구간마다 돌리고 C를 부른다. 무전 게이트도 여기 있다. **DOM을 모른다** |
...
- 무전이 구간을 자르면 **띄워둔 판정을 버리고 한 번 더 부른다.** 판당 최대 2콜이 는다.
...
```bash
npm test        # 규칙 전량 (152항목) — 수치 · 데이터 · 스키마 · 하네스 · 무전 · 하이어아키 감사
