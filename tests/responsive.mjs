...
// 'dossier'는 화면이 아니라 로스터 위에 뜨는 모달이다. show()가 특수 처리한다.
const SCREENS = ['boot', 'intro', 'briefing', 'roster', 'dossier', 'salon', 'interro', 'gate', 'chat', 'result'];
  ...
  await page.evaluate(n => {
    ...
    if (isModal) {
      ...
      [...document.querySelectorAll('.couple-card')]
        .find(el => el.textContent.includes(t.client.name))?.querySelector('.cc-info')?.click();
