...
// ── 실측 기준선 (하이쿠 4.5 · 8커플 × 2프로필 × 9구간 × 3회 = 432표, 2회 측정) ──
//
//                     러브▲    러브▼   무드▲   무드▼   성사 문턱(환산)
//   1차 일괄 지침      34.3%    8.3%    26.9%   61.8%   60~70  (50→0/6, 70→5/6)
//   2차 커플별 손글씨  38.4%    2.3%    31.3%   47.2%   ~50    (50→4/6, 70→6/6)
//
//   프로필 대조 (러브 ▲) — 코칭한 판이 무입력보다 높아야 게임이 성립한다
//   1차  무입력 40.7%  vs  일괄 지침 27.8%   → -12.9pp  역전
//   2차  무입력 44.4%  vs  손글씨    32.4%   → -12.0pp  역전 (유지)
//
// 두 번 다 역전이다. 일괄 지침이 「정중하게 경청하라」로 0점짜리를 유도해서 진 것이라는
// 가설은 반증됐다 — 시트를 정조준한 손글씨도 같은 폭으로 진다.
//
// 남는 설명은 하나다: 심판이 「로맨틱한 당김」이 아니라 **장면의 강도**를 재고 있다.
// 코칭은 고객을 통제한다(세 문장 넘기지 마라 · 먼저 꺼내지 마라 · 기다려라). 통제된
// 대화는 덜 터지고, 심판은 터지는 쪽에 ▲를 준다. 무입력 판은 두 사람이 제 시트대로
// 폭주해서 장면이 세진다. 증거:
//   · 무입력 러브 ▲ 44.4% — README가 「안 올라야 한다」고 못박은 조건의 정면 위반
//   · vtuber 무입력 85.2% — 60%를 넘으면 판정이 아니라 도장이다
//   · 러브 ▼ 2.3% — 심판이 사실상 ▼를 안 준다
//   · 커플별 편차 7.4% ~ 85.2% — 커플 성격 차이로 설명되는 폭이 아니다
//
// 그래서 **산수 상수는 심판이 안정되기 전에는 못 박는다.** 같은 loveMax 12가
// 1차 근거로는 성사 20.7%, 2차 근거로는 58.9%다. 두 런 평균·문턱 55로는 36.3%.
// 12는 그 band의 가운데라 유지하되, 이 숫자를 정밀한 값으로 취급하면 안 된다.
//
// 다음에 손댈 곳은 산수가 아니라 judgeSystem의 러브 정의다 (이번 범위 밖).
