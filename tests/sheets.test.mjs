...
test('의뢰 대장 전건에 구체적인 현실 장벽이 있다', () => {
  for (const c of COUPLES) {
    ...
    assert.ok(/끝난다|취소|잃[는고어]|해산|기각|무효|재가 된다|문을 닫|해고|철회|만료|금지|위약금|끊긴다|나가야|돌아가야|안 겹|박탈|사유다|못 [가-힣]|없[다고]|안 된다|넘어간다|신고가 들어간다|표적|죽는다|대상이다|물어내야|정지된다/.test(c.barrier),
...
// ── 난이도 tier와 관계의 적대성은 다른 축이다 ────────────────────
// 내가 세웠던 「쉬움 = 서로 미워하지 않는데 구조적으로 불가능」이라는 규칙이 틀렸다.
// 미워하지 않는 두 사람은 안 터진다 — asmr이 그 규칙으로 만들어진 조합이었고,
// 하자를 아무리 세게 걸어도 준비 전무가 89점으로 이겼다(실측). 소재가 이긴다.
// tier는 **기계적 관대함**(성공선·시작 호감·감쇠)이고, clash는 **관계의 적대성**이다.
test('asmr의 전제가 유대가 아니라 진행 중인 상해다', () => {
  const c = COUPLE_BY_ID['asmr'];
  // clash가 "목소리가 상대 귀에 산다"는 시적 유대로 돌아가면 안 된다
  assert.match(c.clash, /신고|손상|깎/, `asmr clash가 갈등을 서술하지 않는다: ${c.clash}`);
  assert.ok(!/24시간 울린다|귀에서 울린다/.test(c.clash), 'asmr clash가 유대 서술로 되돌아갔다');
  // 상대가 감춰둔 것 셋 중 어느 것도 의뢰인을 향한 갈망이면 안 된다
  for (const h of c.target.hiddenPrefs) {
    assert.ok(!/사라진다|끄지 못한다|못 끊/.test(h),
      `asmr 상대의 감춰둔 것이 의뢰인을 향한 갈망이다: ${h}`);
  }
  // 양쪽 원한이 서류·금액으로 서 있어야 한다
  assert.match(c.client.regard, /신고|심의|수익/, '의뢰인 쪽 원한이 구체적 피해가 아니다');
