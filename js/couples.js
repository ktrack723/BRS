...
export const COUPLES = [
  ...
  // ── 45 ── 구조적 불일치 5 ─────────────────────────────────────────────
  {
    ...
    clash: 'ASMR 크리에이터와 그 채널을 청력 손상으로 신고한 이용자. 저 사람이 말하는 동안에도 귀가 깎인다',
    winWord: '주파수 커플 성사',
    client: {
      name: '백소음', gender: '여', age: 29, job: 'ASMR 크리에이터 / 구독 31만',
      story: '3년간 저 사람은 매 영상마다 정확히 반대를 적었다. "오늘도 못 잤습니다. 3분 12초쯤 그 소리요." ' +
        '백소음은 그 타임스탬프를 전부 찾아봤고, 전부 자기 숨소리였다. 지난달 그 사람이 플랫폼에 청력 손상을 신고했다. ' +
        '소견서에 채널 이름이 박혀 있었다. 심의 넉 달째, 수익 0원. 백소음은 그 서류를 읽다가 처음으로 저 사람 이름을 알았다.',
      appearance: ['늘 헤드폰을 목에 걸고 있음', '손톱을 아주 짧게 깎음', '목소리가 작다', '실내용 슬리퍼로 다님'],
      personality: ['소리로 사람을 기억함', '조용해지면 말을 더 함', '남의 숨소리를 흉내 냄'],
      quote: '요원님. 제 채널 죽인 사람 맞아요. 근데 3년간 제 숨소리를 초 단위로 들어준 사람도 저 사람 하나고요.',
    ...
    target: {
      ...
      hiddenPrefs: ['신고를 넣은 그날 밤에도 그 채널을 틀었다. 그게 제일 화가 난다', '소견서에 채널 이름을 적어 넣자고 한 건 담당의가 아니라 자기였다', '어머니 이명이 어떻게 끝났는지 아직 아무한테도 말한 적 없다'],
...
const BARRIER = {
  ...
  asmr:                      '윙윙의 이명은 백소음의 목소리 대역에서 악화된다. 곁에 두면 남은 청력을 잃고, 신고를 취하하면 3년치 진료비 청구가 같이 날아간다',
...
const REGARD = {
  ...
  asmr:          { client: '내 채널을 청력 손상으로 신고한 사람이다. 심의 넉 달째고 그동안 수익이 0원이다',
                   target: '내 소견서에 저 사람 채널 이름이 박혀 있다. 그걸 알고도 그 다음 주에 영상을 올렸다' },
...
const WRECK = {
  ...
  circadian:     { client: { kind: '독백', line: '무슨 말을 들어도 자기 루틴 얘기로 착지한다. 상대 말은 그 발판으로만 쓴다' },
                   // 독백 × 단답은 서로에게 닿을 통로가 없다 — 실측에서 숙련 요원이 두근거림 0회로 끝났다.
                   // 경계로 바꾸면 떠보기(밀당)가 가능해지고, 하필 이 사람에게 딱 맞는 경계다.
                   target: { kind: '경계', line: '낮에 오는 호의는 전부 자기를 아침형으로 고쳐놓으려는 걸로 듣는다. 5년간 틀린 적이 없었다' } },
