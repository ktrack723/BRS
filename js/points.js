...
// **성사선까지 몇 칸을 벌어야 하는가**다 — ceil(0.7 × 최대치) − 시작값.
//   판정이 부풀던 시기(코칭 ▲ 43%)에는 20이 필요했고, 초면 전제(15차)에서 12로 내렸다.
//   배우를 둘로 가른 17차에서 코칭 ▲가 25% → 14.1%로 내려가 7칸(2→12)이 너무 멀어졌다.
//   같은 분포로 오프라인 스윕(4만 판)을 돌려 6칸(1→10)으로 맞췄다:
//     7칸(현행 2/12)  무입력 성사 0.6% · 코칭 6.3%   ← 잘해도 못 이긴다
//     6칸(1/10) ✅    무입력 성사 1.0% · 코칭 10.0%
//     5칸(2/10)       무입력 성사 2.2% · 코칭 14.3%  ← 아무것도 안 한 판이 너무 붙는다
//   심판이 다시 부풀면 이 숫자도 같이 올린다. 규칙(달아오름 +1 · 연속 보너스)은 그대로다.
//
...
// 현장 무전 — 고객이 아니라 현장 요원에게 때리는 무전. 물리 지원(꽃다발·차량·연출)을
// 그대로 실행시킨다. 판 전체에 한 번뿐이고, **토킹에서만** 쓸 수 있다.
// 텍스팅에는 현장이 없다: 두 사람이 각자 딴 데서 화면만 보고 있어서 같은 방에 물건을
// 놓을 자리 자체가 없다. 한쪽에 배달해 놓고 전화로 설명하게 만드는 것은 물리 개입이 아니다.
export const FIELD = { perOp: 1, phases: ['talk'] };
...
export const POINTS = {
  ...
  loveStart: 1, loveStep: 1, loveMax: 10,
