...
· 질색한다고 알려진 것: ${t.redLines.join(' / ')}
그 이상은 모른다. 저 사람 속에 뭐가 들었는지는 하나도 모른다.
...
■ 등급 — 기준은 오직 상대의 반응이다. 기본값은 flat이다.
  · breakthrough (+7~+10) — 상대가 실제로 무너졌다. 방어선이 내려갔거나, 감췄던 걸 꺼냈거나,
      판 자체가 뒤집혔다. 한 공작에 한두 번 나올까 말까 한 순간이다.
  · warm (+4~+6) — 상대가 눈에 띄게 호의적으로 움직였다. 말이 길어졌거나, 웃었거나, 되물었다.
  · nudge (+1~+3) — 조금 통했다. 대화는 굴러가고 상대도 싫지 않다. **잘 굴러가는 대화의 대부분이 여기다.**
...
■ 인심 쓰지 마라
  1. 대사가 예쁘다는 이유로 등급을 올리지 마라. 상대가 안 움직였으면 flat이다.
  2. 매 발언은 독립적으로 채점한다. 앞 턴이 좋았다고 이번 턴이 오르지 않는다.
  3. 같은 얘기를 두 번째로 꺼내는 건 처음만큼 오르지 않는다.
  4. 질문을 던졌다는 사실 자체로는 아무 등급도 오르지 않는다. 무엇을 물었고 뭐라고 답이 왔는지가 전부다.

■ moodDelta — 자리의 공기. love와 별개로 매긴다.
  +6 이상은 판이 실제로 뒤집힌 순간에만. 잘 받아친 정도는 +2~+4.
  0 근처는 무난하지만 흐름이 안 느는 상태. 마이너스는 뜬금없거나, 질문을 씹었거나, 혼자 떠들거나, 반복이다.
...
반응을 근거로 삼아라. 상대가 풀어졌으면 그 발언이 통한 것이고,
반응이 짧아지거나 싸늘해졌다면 그 발언이 잘못 들어간 것이다. 반응이 미지근하면 등급도 미지근하다.
